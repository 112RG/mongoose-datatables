...
function dataTablesPlugin (schema, options) {
  options = options || {}
  var totalKey = options.totalKey || 'recordsFiltered'
  ...
  schema.statics.dataTables = function (params, callback) {
    ...
    return new Promise((resolve, reject) => {
      ...
      var countDocuments = thisSchema.count(find)
      if (thisSchema.countDocuments) {
        countDocuments = thisSchema.countDocuments(find)
      }

      Promise
        .all([query.exec(), countDocuments])
